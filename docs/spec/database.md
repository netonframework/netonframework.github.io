...
10. [参考实现](#十参考实现)
11. [结论与建议](#十一结论与建议)
12. [Schema Migration（数据库演进规范）](#十二schema-migration数据库演进规范)
...
## 十二、Schema Migration（数据库演进规范）

> 原为独立的 `migration.md` spec，2026-06-07 合并入本文档。涵盖 migration engine 边界、命令集、history 表 contract、部署 workflow、状态模型、操作员恢复路径等完整规范。
...
### 零、Canonical Architecture

Migration engine lives inside **`neton-database`**. The canonical (and only) execution entry is **`application.kexe migrate`**.

> 历史说明 (2026-06-07):曾存在的独立 `neton-migrate` CLI binary 已于 2026-06-01 logical 弃用,2026-06-07 物理删除 (neton 仓 commit `a6908bc`)。本规范内不再保留 legacy CLI 的命令集 / 配置 / 退出码描述,正式部署一律用 `application.kexe migrate`。
...
- 引入任何 standalone 的 migration CLI 作为 application 部署入口
  (canonical entry 是 `application.kexe migrate`;同进程同 driver,K/N 链接器约束之一)
```
...
| **正式执行入口** | **`application.kexe migrate up | status | verify`**(canonical,唯一入口) |
