...
未知的 codec、非数字的 `ttlMs` / `maxSize`、非布尔的开关都会在启动时报错，不会被悄悄替换成默认值。
...
注意「没设置」与「设成了默认值」是区分开的：模块 DSL 里显式写一个恰好等于默认值的值，
仍然会覆盖文件——`redis { port = 6379 }` 赢过文件里的 `6380`。
...
::: warning TOML 解析器不支持数组
列表型配置（例如 CORS origins）必须通过 DSL 设置。
:::
