...
::: warning 这个文件做不了两件事
1.0 **不支持** `server.host`——监听地址硬编码为 `0.0.0.0`。

Redis **不在这里配**。它读自己的 `config/redis.conf`，键根级平铺、禁止 `[redis]` 段，
遵循「文件名 = 命名空间」约定。`database.conf`、`routing.conf`、`cache.conf` 同理。
:::
...
| **macOS x64** | `macosX64` | 除 `neton-database` 外支持 | 上游驱动 sqlx4k 不发布 `macosX64` artifact |
...
| **限流阈值** | 升级到 1.0.0-beta1 后重新确认每处 `@RateLimit`——更早的版本里它并未真正生效 |
