...
::: tip 登录接口配合限流
登录路由建议同时标注 `@RateLimit`，见[中间件指南](/zh-hans/guide/middleware)。1.0.0-beta1
之前该注解只解析不执行，升级后请重新确认阈值是否真的生效。
:::
...
`neton-security` 在 `neton.security.crypto` 下提供 `Sha256`、`HmacSha256`、`SecretBox`（AES-GCM）、
`SecureRandom` 和 `PasswordHasher`，都是公开 API——签名请求不再需要 import internal 包。
