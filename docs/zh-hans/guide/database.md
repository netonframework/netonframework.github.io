...
「读出来、改完、写回去」在并发下会丢更新。`UpdateScope` 因此提供 `increment` / `decrement`，
生成 `col = col + ?`，让加减在数据库里完成。配合 `where { }` 就是一次 CAS：条件不成立时
影响行数为 0。
...
余额扣减同理：`where { Account::balance ge amount }` 配 `decrement(Account::balance, amount)`。

::: warning 已知边界
谓词只支持「列 vs 绑定值」，不支持列与列相比，所以 `used_count < max_uses` 这种条件仍要写裸 SQL。

本次发布也**没有**幂等插入 API。`ON CONFLICT DO NOTHING` 在 PostgreSQL 和 SQLite 上语义精确，
但 MySQL 上没有一个既不吞掉无关错误、返回值又无歧义的等价写法；只支持两个方言就把一个残缺方法
冻进 1.0 ABI 了。暂时请用 `DbContext` 写裸 SQL。
:::
...
::: warning 带默认值的构造参数不会被注入
`@Logic` 会跳过有默认值的构造参数。写成 `redis: RedisClient? = null` 的可选依赖永远注不进去，
即使绑定存在，而且没有任何警告。依赖请不要写默认值。
:::
...
::: tip 平台说明
`neton-database` 没有 `macosX64` 目标——上游驱动 sqlx4k 不发布该 artifact。
数据库相关开发需要 Apple Silicon、Linux 或 Windows。
:::
...
正式应用禁止在启动阶段调用 `ensureTable()`。它表达不了 ALTER、索引、外键、唯一约束、数据迁移、
版本记录和回滚；schema 演进必须使用版本化 migration SQL。
