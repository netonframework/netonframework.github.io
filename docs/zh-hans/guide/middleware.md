...
| **Rate Limit** | 通过 `RateLimitGate` 执行 `@RateLimit` | neton-http / neton-routing |
...
handleRoute(route, call)
  ...
  |
  +-- 限流预处理：@RateLimit 超限 -> 429 Too Many Requests
...
限流刻意放在认证之后——按用户维度限流时才拿得到调用方身份。

---

## 三、限流

`@RateLimit` 在真实派发路径上执行，位于认证之后、handler 之前：
...
|------|------|
| `windowSeconds` | 窗口长度 |
| `maxRequests` | 每个窗口允许的请求数 |
| `scope` | `IP` 或 `USER`，决定计数按什么维度分组 |
| `key` | 可选的显式 key |
| `message` | 超限时返回的消息 |

v1 只支持 `FIXED_WINDOW` 策略，其他值在编译期报错。超限返回 HTTP 429。

::: warning 从旧版本升级
1.0.0-beta1 之前，这个注解只是解析并生成了元数据，**从未真正生效**——唯一的执行点在 HTTP
adapter 根本不会调用的代码路径上。如果你曾依赖 `@RateLimit` 做登录爆破防护，那期间实际是没有
保护的。升级后请重新确认各处限流阈值。
:::
...
## 六、HttpContext.attributes：请求级自定义数据
...
Neton v1 的中间件能力（安全、限流、日志、追踪）内建于请求处理流程中，满足大多数场景。后续计划引入正式的中间件管道 API，支持：

- 自定义中间件的注册与排序
- 请求/响应的拦截与修改
- CORS 等通用中间件（限流已在 v1 内建，见第三章）
- 中间件的条件执行（按路由、按方法等）
