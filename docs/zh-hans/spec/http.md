...
>
> **配套**：[HTTP 引擎能力规范](./http-engine-capabilities.md) —— 本文描述抽象的**形状**
> （HttpAdapter / HttpContext / 生命周期），但不描述引擎的**能力**。
> 「这个引擎能不能做真流式 / HTTP/2 / multipart」由能力规范定义，
> 并要求在启动期校验、不匹配即失败。今天的默认 `stream()` 会把多次 `writeChunk`
> 静默缓冲成一次 `write`——SSE 在不支持真流式的引擎上不报错但行为错误，
> 正是能力规范要消灭的一类问题。
